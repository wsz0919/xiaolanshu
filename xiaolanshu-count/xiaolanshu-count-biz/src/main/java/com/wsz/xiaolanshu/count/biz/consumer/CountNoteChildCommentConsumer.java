...
import com.wsz.xiaolanshu.count.biz.domain.dto.CountPublishCommentMqDTO;
import com.wsz.xiaolanshu.count.biz.enums.CommentLevelEnum;
import com.wsz.xiaolanshu.count.biz.mapper.CommentDOMapper;
...
public class CountNoteChildCommentConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        for (Map.Entry<Long, List<CountPublishCommentMqDTO>> entry : groupMap.entrySet()) {
            ...
            // 更新 Redis 缓存中的评论计数数据
            // 构建 Key
            String commentCountHashKey = RedisConstants.buildCountCommentKey(parentId);
            // 判断 Hash 是否存在
            ...
            if (hasKey) {
                ...
                redisTemplate.opsForHash()
                        .increment(commentCountHashKey, RedisConstants.FIELD_CHILD_COMMENT_TOTAL, count);
            }

 ...
 *
 * @Author wangshaozhe
 * @Date 2026-02-09 13:34
 * @Company:
 */
@Component
@RocketMQMessageListener(consumerGroup = "xiaolanshu_group_" + MQConstants.TOPIC_COUNT_NOTE_COMMENT, // Group 组
...
public class CountNoteCommentConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        for (Map.Entry<Long, List<CountPublishCommentMqDTO>> entry : groupMap.entrySet()) {
            ...
            // 更新 Redis 缓存中的笔记评论总数
            // 构建 Key
            String noteCountHashKey = RedisConstants.buildCountNoteKey(noteId);
            // 判断 Hash 是否存在
            ...
            if (hasKey) {
                ...
                redisTemplate.opsForHash()
                        .increment(noteCountHashKey, RedisConstants.FIELD_COMMENT_TOTAL, count);
            }
